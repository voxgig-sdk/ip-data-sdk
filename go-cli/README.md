...
boru-driven command-line client **and** interactive REPL for the IpData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-data-cli)
make build
...
./ip-data-cli load 1 get_ip_info            # {id:1} shorthand
./ip-data-cli load '{id:1}' get_ip_info       # explicit match map

# 5. Override the API base URL for a single call
IP_DATA_BASE=https://api.example.com ./ip-data-cli load 1 get_ip_info

# 6. No arguments -> interactive REPL
./ip-data-cli
ip-data> load 1 get_ip_info
ip-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ip-data-cli load 1 get_ip_info
   ```
...
```sh
./ip-data-cli load 1 get_ip_info          # scalar shorthand for {id:1}
./ip-data-cli load '{id:1}' get_ip_info     # explicit match map
```
...
./ip-data-cli load 1 get_ip_info
```
...
Run with no arguments to open a REPL (prompt `ip-data>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ip-data-cli`. |
...
(`./ip-data-cli <expr>`) and interactively (the REPL), and expressions compose the
