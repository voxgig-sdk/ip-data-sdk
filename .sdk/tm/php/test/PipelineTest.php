...
{
    private static function utility(): IpDataUtility
    {
        return new IpDataUtility();
    }
    ...
    private static function ctx(array $over = []): IpDataContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new IpDataContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new IpDataOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof IpDataError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new IpDataOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = IpDataMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new IpDataOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = IpDataMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new IpDataOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = IpDataMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IpDataSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = IpDataMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new IpDataResult([]),
        ]);
        [, $err] = IpDataMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new IpDataSpec(['step' => 's']),
            'response' => new IpDataResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new IpDataResult([]),
        ]);
        [, $err] = IpDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new IpDataSpec(['step' => 's']),
            'response' => new IpDataResponse(self::resp(200, ['v' => 1])),
            'result' => new IpDataResult([]),
        ]);
        [, $err] = IpDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new IpDataSpec(['step' => 's']),
            'response' => new IpDataResponse(self::resp(200, ['v' => 2])),
            'result' => new IpDataResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        IpDataMakeResponse::call($ctx);
    ...
    {
        $preset = new IpDataResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new IpDataSpec([]),
            'response' => new IpDataResponse([]),
            'result' => new IpDataResult([]),
        ...
        [$got, $err] = IpDataMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new IpDataResult([])]);
        [, $err] = IpDataMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new IpDataSpec([]), 'result' => null]);
        [, $err] = IpDataMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IpDataSpec(['step' => 's']),
            'result' => new IpDataResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new IpDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IpDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IpDataSpec(['step' => 's']),
            'result' => new IpDataResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new IpDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IpDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IpDataResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new IpDataSpec([]),
            'result' => new IpDataResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = IpDataMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IpDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new IpDataError('boom', 'boom');
        $utility->fetcher = function (IpDataContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IpDataSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = IpDataMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (IpDataContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = IpDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(IpDataResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (IpDataContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IpDataSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = IpDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new IpDataResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new IpDataSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = IpDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = IpDataMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = IpDataMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new IpDataResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, IpDataDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new IpDataResult(['ok' => false])]);
        $this->expectException(IpDataError::class);
        IpDataDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new IpDataResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', IpDataMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new IpDataBaseFeature();
        $b = new IpDataBaseFeature();
        IpDataFeatureAdd::call($ctx, $a);
        IpDataFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): IpDataBaseFeature
    {
        $f = new IpDataBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        IpDataFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): IpDataContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new IpDataSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = IpDataPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new IpDataResponse(['headers' => null]),
            'result' => new IpDataResult([]),
        ]);
        IpDataResultHeaders::call($ctx);
