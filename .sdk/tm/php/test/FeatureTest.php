...
class FtCtrl extends IpDataControl
{
...
class FtClient
{
    ...
    private IpDataUtility $utility;

    public function __construct(IpDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): IpDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IpDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpDataConfig::make_feature($name);
    ...
    private static function buildUrl(IpDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpDataResponse($fetched);
        }
    ...
    private function fail(IpDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
