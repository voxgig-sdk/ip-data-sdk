...
//     (\`new IpDataSDK(...)\` / \`IpDataSDK.test(...)\` become
//     \`IpDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new IpDataSDK(...)\` and any
// \`IpDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IpDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IpDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
