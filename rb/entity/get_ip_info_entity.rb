...
class GetIpInfoEntity
  ...
  def data_set(args)
    if args
      @_data = IpDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = IpDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [GetIpInfo, Hash] the loaded GetIpInfo; raises IpDataError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpDataHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
