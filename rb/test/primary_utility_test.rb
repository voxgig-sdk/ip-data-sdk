...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = IpDataOmni.make_runner(TEST_JSON_FILE, IpDataSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : IpDataOmni.errify(err)
    end
