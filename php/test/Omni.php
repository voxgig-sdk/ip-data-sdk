...
//    only - a typed IpDataContext there would make every ctx
...
final class IpDataOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new IpDataContext($clean, null);
    ...
    public static function err_from_map($m): ?IpDataError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new IpDataError(is_string($code) ? $code : '', $msg);
    }
