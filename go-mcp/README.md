...
[MCP](https://modelcontextprotocol.io) server exposing the IpData SDK as
two agent tools — `ip-data_list` and `ip-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-data-mcp)
make build

# 2. Provide credentials via the environment
export IP_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ip-data \
  -- /absolute/path/to/ip-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ip-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ip-data_list: first page of records
{ "entity": "get_ip_info" }
{ "entity": "get_ip_info", "query": { } }

// ip-data_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ip-data \
     -- "$PWD"/dist/*/ip-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ip-data_list` and `ip-data_load` tools now appear
   in new sessions. Ask the agent to *"list get_ip_info using ip-data"*
   and it calls `ip-data_list` with `{"entity":"get_ip_info"}`.
...
```sh
export IP_DATA_APIKEY=sk_live_xxx            # API key
export IP_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ip-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IP_DATA_APIKEY` | API key sent with every request. |
| `IP_DATA_BASE` | Optional override of the API base URL. |
...
```sh
./ip-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ip-data_load","arguments":{"entity":"get_ip_info","query":{"id":1}}}}'
```
