...
MCP server exposing the IpData SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-data_list` | `entity`, optional `query` map | First page of records as JSON |
| `ip-data_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `ip-data_list` and
`ip-data_load` tools become available in new sessions.
